...
\begin{frame}{Critical Pairs}
    \rui{Is this too much text for one slide?}
  \begin{block}{Definition}
    \vspace{0.3em}
    A \textbf{critical pair} of a thread is a pair \((X, \ell)\) such that some
execution of the thread acquires an unheld lock \(\ell\) while already
holding the set of locks \(X\).
  \end{block}
  \begin{block}{Deadlock Condition}
    \vspace{0.3em}
    For the case of two threads:
    \begin{align*}
    C1 \mid \mid C2 \quad deadlocks \iff \text{there are critical pairs } \\ (X1, \ \ell1) \text{ and } (X2, \ \ell2) \text{ of C1 and C2 respectively, such that:}
    \end{align*}
    \[
    \ell1 \in X2 \quad and \quad \ell2 \in X1 \quad and \quad X1 \cap X2 = \emptyset
    \]
  \end{block}
\end{frame}

\begin{frame}{Example of Critical Pairs}
  \begin{exampleblock}{Example}
    ...
    \end{align*}
    Their critical pairs are:
    \begin{align*}
    Crit(C1) = \{(\emptyset, x), (\{x\}, y)\} \\
    Crit(C2) = \{(\emptyset, y), (\{y\}, x)\}
    \end{align*}
  \end{exampleblock}
  Since \(x \in \{x\}\), \(y \in \{y\}\) and \(\{x\} \cap \{y\} = \emptyset\), the deadlock condition holds — \alert{deadlock!}
...
\begin{frame}{Example of Critical Pairs}
  \begin{exampleblock}{Solution}
    ...
    Their critical pairs are:
    \begin{align*}
    Crit(C1') = \{(\emptyset, z), (\{z\}, x)\}, (\{z,x\}, y)\} \\
    Crit(C2') = \{(\emptyset, z), (\{z\}, y)\}, (\{z,y\}, x)\}
    \end{align*}
  \end{exampleblock}
  \vspace{-1em}
  Since \(\{z,x\} \cap \{z,y\} = z\), the deadlock condition does not hold — \alert{no deadlock!} \\
  \(z\) acts as a guard lock, preventing x and y from being accessed by C1' and C2' simultaneously.
\end{frame}
...
\begin{frame}{Abstract Language}
  \begin{itemize}
    ...
    \end{align*}
    \vspace{-3em}
    \item This grammar guarantees balanced pairs of lock acquisitions/releases.
    \item We consider only non-recursive procedures.
    \item If $C$ is balanced, then so is body($p$) with $p \in$ callees(C).
    \item A parallel program is an n-tuple of balanced statements \( C_1 \,||\, C_2 \,||\, ... \,||\, C_n \).
  \end{itemize}
\end{frame}
...
\begin{frame}{Lock States}
  \begin{itemize}
    \item We treat locks as \textbf{re-entrant and scoped}.
    \item A $lock$ $state$ is a function that maps each lock to its acquisition count.
    \item A $configuration$ is a pair $\langle C,L \rangle$, where $C$ is a statement and $L$ is the lock state.
    \item A $concurrent$ $configuration$ is then, a pair of the form $\langle C_1 || ... || C_n, (L_1, ..., L_n) \rangle$.
  \end{itemize}
\end{frame}
...
\begin{frame}{Proof Structure}
  ...
  \begin{block}{Result}
    \vspace{0.3em}
    Existence of deadlock $\Leftrightarrow$ conflict between critical pairs.
  \end{block}
\end{frame}
